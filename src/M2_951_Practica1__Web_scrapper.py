...
def descarrega_pagina(url, intents = 5, timeout = 1, agent_usuari = 'ua0000', retorna = 'text'):
    """
    Funció que obté els continguts del lloc web url, si aquest no està desabilitat al fitxer robots.txt
    ...
    Retorna:
        html           : dades obtingudes com a resposta del lloc url a la petició GET. No tenen perquè ser codi html
        codi_error     :
    ...
    if not robot.can_fetch(agent_usuari, url):
        ...
        return html, codi_error, missatge_error
    ...
    if (intents>0) and (500<=pagina.status_code<600):
        ...
        return descarrega_pagina(url, intents = intents-1, timeout = timeout)
    ...
    # Finalment, si no hi ha hagut errors, retornem un codi d'error 0 i la pàgina web en format
    # html. Si retorna val 'text', es retorna el contingut en format text. Sinó (si val 'binari',
    # per exemple), es retorna el contingut binari
    html = pagina.text if retorna == 'text' else pagina.content
    codi_error = 0
    missatge_error = None
    return html, codi_error, missatge_error
...
def crawler_upc_graus():
    ...
    Retorna:
        ...
        codi_error     : codi d'error de la funció descarrega_pagina()
        missatge_error : missatge d'error de la funció descarrega_pagina()
    """
    
    html, codi_error, missatge_error = descarrega_pagina('https://www.upc.edu/ca/graus/',
...
def scraper_upc_graus(adreca_web_grau, verbose = True):
    """
    Funció que obté, a partir de l'adreça de la pàgina web d'un grau oficial  
    de la UPC,la informació rellevant sobre el mateix.
    
    Paràmetres:
        adreca_web_grau : adreça de la pàgina web del grau
    ...
    Retorna:
        grau: diccionari amb la informació recopilada amb el format següent:
                { 'Nom' :             nom del grau
                  'Adreça web' :      adreça de la pàgina web del grau
                  ...
                  'Assignatures' : [ { 'Nom' :             nom de l'assignatura,
                                       ...
                                       'Adreça web' :      adreça de la pàgina web de l'assignatura,
        ...
        codi_error : 0 si tot és correcte. Sinó, el codi d'error heretat de
                     descarrega_pàgina() o -4 si no ha pogut obtenir ni el
                     nom del grau
    ...
    html_aux, codi_error, missatge_error = descarrega_pagina(adreca_web_grau, timeout = 10,
                                           retorna = 'binari')
    ...
    if codi_error:
        if verbose:
            print("No s'ha pogut descarregar la informació del lloc web "+adreca_web_grau)
    ...
    try:
        ...
        if verbose:
            print(nom.upper())
        grau['Adreça web'] = adreca_web_grau
    except:
        # Si hi ha algun problema abandonem la funció sense retornar res
        if verbose:
            print("No s'ha pogut resoldre el nom del grau a " + adreca_web_grau)
        grau = {}
        codi_error = -4
        missatge_error
        return grau, codi_error, missatge_error
    ...
    try:
        ...
        if not tags_semestres:
            ...
            # d'error de descarrega_pagina que, si s'ha arribat ací, és 0 (sense error)
            return grau, codi_error, missatge_error
    except:
        ...
        try:
            ...
            for assignatura in tags_assignatures:
                ...
                grau['Assignatures'].append({'Nom': nom,
                                             ...
                                             'Adreça web': adreca_web,
                                             'Tipus': tipus,
                                             'Menció': mencio})
            # Per a graus amb mencions, eliminem duplicats d'assignatures comunes a TOTES les mencions
            if mencions:
                i = 0
                while i<len(grau['Assignatures']):
                    ...
                    posicions_duplicats = [j for j in range(i+1, len(grau['Assignatures'])) if (
                                           ...
                                           grau['Assignatures'][j]['Adreça web'] == grau['Assignatures'][i]['Adreça web'])]
...
webs_graus, codi_error, missatge_error = crawler_upc_graus()
...
with open('dades_graus_upc.csv', 'w', newline='') as f:
    ...
    for w in webs_graus:
        temp.espera()
        dades, codi_error, missatge_error = scraper_upc_graus(w)
